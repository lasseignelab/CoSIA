...
Test2 <- new("CosiaAnnotate", input = FullHumanMDLKD$Human_Symbol, input_species = "homo_sapiens", input_id = "HGNC.Symbol", output_ids = c("Ensembl.id","Entrez.id"), output_species = "mus_musculus", tool = "biomaRt", ortholog_database = "HomoloGene")
Output2<-getConversion(Test2)
head(Output2)
...
SGO1_expression_across_tissues <- new("CosiaExpressSpecies",list_of_ensembl_ids = c("ENSDARG00000103907"), list_of_respective_species = c("danio_rerio"), single_tissue = "kidney")
...
```{r}
library(CoSIA)
conversion_test <- new("CosiaAnnotate",
                              input = "NGN1", 
                              input_species = "homo_sapiens", 
                              input_id = "Symbol", 
                              output_ids = "Ensembl.id",
                              output_species = "homo_sapiens", 
                              tool = "biomaRt")

test_dataframe <- getConversion(conversion_test_one)


conversion_test_one <- new("CosiaAnnotate",
                       input="SGO1",
                       input_species="homo_sapiens",
                       output_species="mus_musculus",
                       input_id=c("Gene.name"),
                       output_ids="Ensembl.id",
                       tool="annotationDBI")
