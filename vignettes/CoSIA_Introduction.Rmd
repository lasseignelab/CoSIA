...
map_tissues = c("adult mammalian kidney", "heart", "brain"),
metric_type="DS_Gene"
)
kidney_ensembl<-getConversions(kidney)
k<-kidney_ensembl@converted_id
kidney_gex<-getGEx(kidney_ensembl)
plotSpeciesGEx(object = kidney_gex, single_tissue = "adult mammalian kidney", single_gene = "ENSG00000008710")
plotTissueGEx(object = kidney_gex, single_species = "m_musculus", single_gene = "ENSG00000008710")
kidney_gex<-getGExMetrics(kidney_ensembl)
CoSIA::plotDSGEx(kidney_gex)
...
```{r}
filter_mouse<-dplyr::filter(Experimental_Hub_File,Species == "Mus_musculus")
filter_mouse<-dplyr::select(filter_mouse, Ensembl_ID, Median_VST, Anatomical_entity_name)
filter_mouse<-unique(filter_mouse)
filter_mouse$VST_Scaled <-((filter_mouse$Median_VST)-min(filter_mouse$Median_VST))/ (max(filter_mouse$Median_VST)-min(filter_mouse$Median_VST))
filter_mouse<-dplyr::select(filter_mouse, Ensembl_ID, Median_VST, Anatomical_entity_name)

filter_gex<- filter_mouse%>% tidyr::pivot_wider(names_from = Ensembl_ID, values_from = Median_VST )
filter_gex <- filter_gex %>% remove_rownames %>% tibble::column_to_rownames(var="Anatomical_entity_name")

hist(filter_mouse$Median_VST)

library(BioQC)
ENTROPY_DIVERSITY_G<-data.frame(entropyDiversity(filter_gex,norm = TRUE)) # across genes
ENTROPY_DIVERSITY_T<-data.frame(entropyDiversity(t(filter_gex),norm = TRUE)) # across tissue
ENTROPY_SPECIFICITY_G<-data.frame(entropySpecificity(t(filter_gex),norm = TRUE)) # across genes
ENTROPY_SPECIFICITY_T<-data.frame(entropySpecificity(filter_gex,norm = TRUE)) # across tissue

hist(ENTROPY_SPECIFICITY_T$entropySpecificity.filter_gex..norm...TRUE.)

```

```{r}
map_species<- c("Homo_sapiens", "Mus_musculus", "Rattus_norvegicus")
...
CV_Species<-filter_gex %>% dplyr::group_by(Ensembl_ID, Species) %>% dplyr::summarise(CV_Species = CV_function(VST, na.rm=FALSE))
