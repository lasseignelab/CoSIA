...
Species<-filter_gex %>% group_by(Gene.ID,Anatomical.entity.name) %>% summarise(Sample_size = n(), Median_VST = median(VST), VST = paste(unique(VST), collapse = ', '))
Species<- dplyr::select(Species, Anatomical.entity.name, Median_VST, Gene.ID)

filter_gex<- Species%>% tidyr::pivot_wider(names_from = Gene.ID, values_from = Median_VST )
