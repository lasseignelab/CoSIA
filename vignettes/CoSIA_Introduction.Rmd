...
filter_mouse<-dplyr::select(filter_mouse, Ensembl_ID, VST_Scaled, Anatomical_entity_name)

filter_gex<- filter_mouse%>% tidyr::pivot_wider(names_from = Ensembl_ID, values_from = VST_Scaled )
filter_gex <- filter_gex %>% remove_rownames %>% tibble::column_to_rownames(var="Anatomical_entity_name")

hist(filter_mouse$VST_Scaled)
