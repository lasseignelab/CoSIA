...
#comment **this isn't how CoSIA is currently organized (based on image as well). Will rewrite text based on it's current shape, keep what you'd like.** 
...
#comment **suggested rewrite/arrangement if maintaing the modules as they are**
The first module and second modules have a shared method, `getConversions`, that is used to conduct conversion between different gene identifiers in the same species as well as in different species. The third and fourth modules have a shared method, `getGEx`, used to obtain raw [VST transformed] gene expression values of a single gene *across multiple tissues in single model organism* or *across multiple model organisms in a single tissue*. Plotting methods, `plotTissueGEx` & `plotSpeciesGEx`, are used to visualize gene expression values for modules three and four, respectively. The fifth module uses the method `getGExMetrics` to calculate median-based Coefficient of Variation and Shannon Entropy. There are two accompanying plotting methods, `plotCVGEx` & `plotDSGEx` that are used to visualize the variation and diversity of gene expression across tissues and model organisms.
...
monogenic_kidney_genes <- readRDS("../data/monogenic_kidney_genes_toy.RDS")
...
| input_id; output_ids | Ensembl_id, Entrez_id, Ensembl_id_version, Gene_name, Symbol (**annotationDBI only**) | N/A |
...
The following use cases provide running examples of CoSIA applications with [Natera's Monogenic Kidney Disease Panel](https://www.natera.com/resource-library/renasight/385-genes-associated-with-monogenic-disorders-linked-to-kidney-disease). We will perform id conversion, obtain gene expression data, visualization of the variation and diversity of gene expression across three species (human, mouse, & rat) and two tissues (kidney & heart). 
...
CoSIAn_Obj_DS <- CoSIA::getConversions(CoSIAn_Obj_DS)

CoSIAn_Obj_DS <- CoSIA::getGEx(CoSIAn_Obj_DS)
