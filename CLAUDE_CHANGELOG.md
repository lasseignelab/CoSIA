...
- **Fixed `CV_Tissue` single-species bug** (`R/CoSIA-getGExMetrics-method.R`) — the `dplyr::filter` referencing `Anatomical_entity_name.x` only exists when multiple species are merged (creating `.x`/`.y` suffixes). Guarded the filter with a column existence check so single-species calls no longer crash.
