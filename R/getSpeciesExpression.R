setMethod("getSpeciesExpression", signature(object = "CosiaExpressSpecies"), function(object) { # user's input of the function
  ...
  x<-cbind(data.frame(list_of_ensembl_ids), data.frame(list_of_respective_species)) #combine the two arguments ensembl id list and the species that correlate with it into a data frame to parse through.
  for (val in 1:nrow(x)) { #takes about 10 mins to run this entire for loop if the data has already been downloaded to the location of your choice.
    ...
    bgee_species <- BgeeDB::Bgee$new(species = gene_species, dataType= "rna_seq", pathToData = pathToData) # call bgee data based on the species and download the data to the pathToData location
    ...
    tissue_specific_data<-dplyr::filter(gene_specific_data,Anatomical.entity.name %in% single_tissue) #filter by the specific anatomical entitiy name in the single tissue
  ...
  if (dataframe==TRUE){
    save(df_1, file = pathToData) # saves the data frame
  }
  if (figure==TRUE){
    ...
    fig<-fig %>% plotly::add_trace(marker = list(size = 8,line = list(color = 'black',width = .75)),
                                   showlegend = F)
    fig <- fig %>%
      plotly::layout(title = stringr::str_wrap(paste("Gene Expression of the gene", list_of_ensembl_ids[1], "in", single_tissue, sep=" ")),
                     xaxis = list(title = "Anatomical Entity Name",size = 2),
                     yaxis = list(title = "TPM (transcript per million)",zeroline = F),
                     showlegend=FALSE)
    return(fig) # return the figure
  }
  else{ # warning stating that if there is no figure returned it can be because of these two things.
    stop("Either you have set both dataframe and figure as FAlSE, or you Dataframe has been loaded. Please check the location that you have set as your PathtoData and make sure that the dataframe is saved there.")
  }
}
)