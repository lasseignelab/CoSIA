...
#'
#' @return initializes a generic function for getGEx as preparation for defining the getGEx Method
#' @export
#' @examples
#' Kidney_Genes <- CoSIAn(
...
setMethod("getGEx", signature(object = "CoSIAn"), function(object) {
    ...
    return_filtered_Gex_data <- function(map_species) {
        ...
        if (any(map_species == "m_musculus_ensembl_id")) {
            ...
            gene_specific_data <- dplyr::filter(mm_EH_File, Ensembl_ID %in% m_ensembl_id)
