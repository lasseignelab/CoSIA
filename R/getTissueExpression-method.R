setMethod("getTissueExpression", signature(object = "CosiaExpressTissue"), function(object) { # user's input of the function
    if (object@tissues=="all tissues"){
      ...
      values<-aggregate(data = gene_specific_data,x = gene_specific_data$TPM,
                        by = list(gene_specific_data$Anatomical.entity.name),
                        FUN = median)
      ...
      fig <- plotly::plot_ly(gene_specific_data, x = ~AEN, y = ~TPM, type = 'scatter', mode = 'markers', color = ~AEN, colors = color) %>%
        plotly::add_markers(x=~ AEN,y=~x ,marker = list(
          symbol = 'line-ew' ,
          size = 10,
          line = list(
            color = "grey",
      ...
      fig <- fig %>%
        plotly::layout(
          ...
          yaxis = list(
            title = "TPM (transcript per million)",
            zeroline = F
          ),
          showlegend=FALSE
        )
      ...
      values<-aggregate(data = tissue_specific_data,x = tissue_specific_data$TPM,
                        by = list(tissue_specific_data$Anatomical.entity.name),
                        FUN = median)
      colnames(values)[which(names(values) == "Group.1")] <- "Anatomical.entity.name"
      value<- merge(values,sample_size,by="Anatomical.entity.name")
      value$AEN <- paste(value$Anatomical.entity.name,"(n=",value$Freq,")", sep = "")
